...
app.post('/api/image/generate', auth.authMiddleware, generationLimiter, upload.array('refs', 10), async (req, res) => {
  ...
  const selectedModel = model || 'pony-v6';
  ...
  const modelInfo = IMAGE_MODELS[model || 'pony-v6'] || IMAGE_MODELS['pony-v6'];
  ...
  const cost = generation.getModelCost(model || 'pony-v6');
  ...
  for (let i = 0; i < count; i++) {
    if (!user.unlimited && !await credits.deductCredits(email, cost, model || 'pony-v6')) break;
    ...
    if (!generation.enqueueGeneration({ taskId, batchId, prompt: finalPrompt, model: model || 'pony-v6', ratio, resolution, refFiles: taskRefs, email })) {
  ...
  await userActivity.logActivity(email, 'generation.create', req, { model: model || 'pony-v6', count: taskIds.length, cost: acceptedCost, taskIds });
  db.query('INSERT INTO prompt_history(email, prompt, model, task_id) VALUES($1,$2,$3,$4)', [email, finalPrompt, model || 'pony-v6', taskIds[0]]).catch(() => {});
